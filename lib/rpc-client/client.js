...
var proxyCB = function(client, serviceName, methodName, args, attach, isToSpecifiedServer) {
  ...
  if (isToSpecifiedServer) {
    ...
    getRouteTarget(client, serverType, msg, routeParam, function(err, serverIdOrErr) {
      if(!!err) {
        utils.invokeCallback(cb, err , serverIdOrErr);
      } else {
        client.rpcInvoke(serverIdOrErr, msg, cb);
      }
    });
  }
};
